...
**✅ 已确认 2026-04-18**：**不做**，节省约 2 天，MVP 为纯静音游戏，列入 Post-MVP 待办。

**落地影响**：
- Sprint 6 范围内去除所有音效/BGM 相关任务
- `autoload/AudioBus.gd` 在 S1 阶段 **暂不创建**（避免未被使用的死代码；如 Post-MVP 决定做音效再补）
...
<details>
<summary>决策背景（原选项对比，仅供追溯）</summary>

- 选项 1：做（+2 天工作量）：主菜单 BGM + 战斗 BGM + 攻击/治疗/UI 音效
- 选项 2：不做（保持 S6 = 2 天）：MVP 纯静音
- Manager 采纳选项 2：MVP 聚焦战斗循环验证，音效对核心玩法影响小
...
**✅ 已确认 2026-04-18**：**不做**，节省约 1 天，MVP 用 Tiny Swords sprite 头像占位，列入 Post-MVP 待办。

**落地影响**：
- Sprint 6 范围内去除"用 Stable Diffusion 生成立绘 + 替换 UI 头像"任务
- MVP 阶段 UI 头像直接用 Tiny Swords 对应 sprite 的 Idle 首帧裁剪作为占位
- sprite 头像清晰度足以区分徐凤年 / 姜泥 / 李淳罡

**Post-MVP 待办**：
- AI 生成 3 位角色 512×512 立绘
- 替换 CharacterSelect / BattleUI / Victory 场景中的头像

<details>
<summary>决策背景（原选项对比，仅供追溯）</summary>

- 选项 1：做（+1 天工作量）：Stable Diffusion 生成立绘替换 Human Avatars
- 选项 2：不做：用 Tiny Swords Human Avatars 占位
- Manager 采纳选项 2：MVP 聚焦战斗循环，立绘对核心玩法影响小
...
**✅ 已确认 2026-04-18**：**保持 6 个 Sprint × 1-2 天**（当前方案，见 [06-sprint-plan.md](./06-sprint-plan.md)），总工作量 10-12 天。

**理由**：
- 粒度适中，DoD 验收频率够高
- 不拆成 8 个（避免 DoD 上下文碎片化）
- 不合并成 4 个（避免单 Sprint 风险过大）

<details>
<summary>决策背景（原选项对比，仅供追溯）</summary>

- 选项 1：保持 6 个 Sprint（当前方案）
- 选项 2：拆成 8 个 Sprint × 1 天
- 选项 3：合并成 4 个 Sprint × 2-3 天
- Manager 采纳选项 1：平衡粒度和验收频率
...
| D2: 音效/BGM | — | ✅ **已确认 2026-04-18**：MVP 不做，Post-MVP 待办 | 见本文 §3 D2 |
| D3: 角色立绘替换 | — | ✅ **已确认 2026-04-18**：MVP 不做，Post-MVP 待办 | 见本文 §3 D3 |
| D4: Sprint 粒度调整 | — | ✅ **已确认 2026-04-18**：保持 6 个 Sprint × 1-2 天 | 见本文 §3 D4 |

---

## 5. Manager 决策全部确认（2026-04-18）

✅ **D1: BOSS 角色设定** — 杨元赞（北莽南朝大将军），Red Warrior sprite 改色 + 节点 scale 1.15，技能"横扫千军"（十字 2 格 AOE）
✅ **D2: Sprint 6 音效/BGM** — MVP 不做，Post-MVP 待办
✅ **D3: 角色立绘替换** — MVP 不做，Post-MVP 待办
✅ **D4: Sprint 粒度** — 保持 6 个 Sprint × 1-2 天

总工作量保持 **10-12 天**（6 个 Sprint × 1-2 天）。

---

**下一步**：开始 Sprint 1 实施（详见 [06-sprint-plan.md](./06-sprint-plan.md) §Sprint 1）。
